...
*{box-sizing:border-box}body{margin:0;background:#fff;font-family:"Microsoft YaHei","Noto Sans CJK SC",sans-serif;color:#172033}.card{width:720px;padding:20px;background:linear-gradient(145deg,#fff,#f1f5f9);border-top:6px solid #3b82f6}.brand{font-size:12px;letter-spacing:1.2px;color:#3b82f6;font-weight:700}.title{font-size:24px;font-weight:800;margin:8px 0 3px}.meta{font-size:13px;color:#64748b;margin-bottom:14px}.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.item{min-width:0;padding:12px 14px;background:#fff;border:1px solid #dbe4f0;border-radius:9px}.rank{font-size:13px;color:#3b82f6;font-weight:700}.item-title{margin:3px 0 6px;font-size:16px;font-weight:700;line-height:1.3}.hot{font-size:13px;color:#475569}.url{margin-top:3px;font-size:11px;line-height:1.25;color:#2563eb;white-space:nowrap}</style></head><body><main class="card"><div class="brand">UAPI · ASTRBOT</div><div class="title">{{ platform }} 热榜</div>{% if update_time %}<div class="meta">更新时间：{{ update_time }}</div>{% endif %}<section class="grid">{% for item in items %}<article class="item"><div class="rank">{{ item.rank }}</div><div class="item-title">{{ item.title }}</div>{% if item.hot_value %}<div class="hot">热度：{{ item.hot_value }}</div>{% endif %}{% if item.url %}<div class="url">{{ item.url }}</div>{% endif %}</article>{% endfor %}</section></main></body></html>'''
...
class UAPIPlugin(Star):
    ...
    async def _call_api(
        ...
        if isinstance(data, (dict, list)):
            if is_hotboard and isinstance(data, dict):
                ...
                if isinstance(items, list):
                    ...
                    display_items = []
                    for position, item in enumerate(items[:hotboard_display_limit], start=1):
        ...
        if is_hotboard or is_search:
            try:
                image_path = await html_renderer.render_custom_template(
                    HOTBOARD_TEMPLATE if hotboard_template_data else PRESENTATION_TEMPLATE,
                    hotboard_template_data
                    or {
                ...
                if hotboard_template_data:
                    with PILImage.open(image_path) as rendered_image:
                        rendered_image.crop(
                            (0, 0, min(rendered_image.width, 720), rendered_image.height)
...
def _make_tool_instance(api: dict, client: UAPIClient):
    ...
    if api_name == "misc.hotboard":
        parameters_schema["properties"]["display_limit"] = {
            "type": "integer",
            "description": "图片中展示的热榜条数，范围 1-50，默认 20。",
            "default": 20,
            "minimum": 1,
            "maximum": 50,
        }
    ...
    class _DynamicTool(FunctionTool[AstrAgentContext]):
        ...
        async def call(
            ...
            query_args = {}
            body_args = {}
            display_limit = _hotboard_display_limit
            ...
            for key, value in kwargs.items():
                if key in ("self", "context", "cls"):
                    continue
                if api_name == "misc.hotboard" and key == "display_limit":
                    try:
                        display_limit = max(1, min(50, int(value)))
                    except (TypeError, ValueError):
                        return "error: display_limit must be an integer between 1 and 50."
                    continue
                    ...
                    if isinstance(data, (dict, list)):
                        ...
                        if api_name == "misc.hotboard" and isinstance(data, dict):
                            ...
                            if isinstance(items, list):
                                ...
                                update_time = data.get("update_time", "")
                                display_items = []
                                for position, item in enumerate(items[:display_limit], start=1):
                                    if not isinstance(item, dict):
                                        continue
                                    title = str(item.get("title", "未命名条目")).replace(
                                        "\n", " "
